...
import { FileText, Download, Loader2, ChevronDown, X, ArrowRight, Languages, Copy, Check, Printer } from 'lucide-react';
...
export default function DocumentGenerator() {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      toast.error('PDF EXPORT FAILED — try Copy Text instead');
    } finally {
  ...
  const handleCopyText = async () => {
    ...
    try {
      const fullText = [
        ...
        `FIR No.: ${generatedDoc.fir_number || '[___]'}`,
        `Language: ${generatedDoc.language === 'hi' ? 'Hindi' : generatedDoc.language === 'gu' ? 'Gujarati' : 'English'}`,
        `Sections: ${(generatedDoc.sections_used || []).join(', ') || 'N/A'}`,
        '',
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <body>
        <h1>${generatedDoc.title.toUpperCase()}</h1>
        <div class="meta">FIR: ${generatedDoc.fir_number || '[___]'} &nbsp;|&nbsp; Generated: ${new Date().toLocaleDateString('en-IN')} &nbsp;|&nbsp; Sections: ${(generatedDoc.sections_used || []).join(', ') || 'N/A'}</div>
        <pre>${generatedDoc.content.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</pre>
        <div class="footer">Generated by CrimeGPT — AI-Powered Crime Documentation System | CONFIDENTIAL — FOR OFFICIAL USE ONLY</div>
      </body></html>
    `);
            ...
            <div className="flex items-start justify-between mb-6">
              ...
              <div className="flex gap-2 flex-wrap">
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 font-bold text-sm uppercase tracking-tighter bg-accent text-background hover:bg-foreground transition-all disabled:opacity-50"
                  title="Download as government-formatted PDF"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 font-bold text-sm uppercase tracking-tighter border-2 border-foreground/50 text-foreground/80 hover:bg-foreground hover:text-background transition-all"
                ...
                <button
                  ...
                  className="p-2.5 text-muted-foreground hover:text-accent transition-colors border border-border"
                >
                  <X size={18} />
                </button>
              </div>
