...
import { Mic, FileText, ArrowRight, Loader2, X, Trash2, Download } from 'lucide-react';
...
export default function FIRAutomator() {
  ...
  const handleExportToWord = () => {
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div style="margin-top: 40px; font-size: 8pt; color: #888; text-align: center; border-top: 1px solid #eee; padding-top: 10px;">
          CONFIDENTIAL — FOR LAW ENFORCEMENT USE ONLY. SUBJECT TO VERIFICATION PER POLICE PROTOCOL.
        </div>
      </body>
      </html>
    `;
    ...
    link.href = url;
    link.download = `${previewFirNumber}_${formData.complainant_name || 'Draft'}.doc`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    toast.success('FIR DRAFT EXPORTED TO WORD');
  };
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-2.5 bg-accent text-background font-bold text-xs uppercase tracking-tighter hover:bg-foreground transition-all"
                  >
                    <Download size={14} />
