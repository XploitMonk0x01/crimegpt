...
Handles:
    - generate_draft() — AI-powered FIR drafting (mock for Phase 4, real AI in Phase 7)
...
class FIRService:
    ...
    async def generate_draft(
        ...
        Phase 4: Returns a structured mock response.
        Phase 7: Will integrate NER -> RAG -> LLM pipeline.
        """
        logger.info(f"Generating FIR draft for officer {officer.badge_no}")

        # --- Phase 4: Mock AI response ---
        mock_sections = ["BNS Section 303 (Theft)", "BNS Section 304 (Snatching)"]
        mock_narrative = (
            f"Based on the incident described, a First Information Report is hereby drafted.\n\n"
            f"INCIDENT SUMMARY:\n{request.incident_description}\n\n"
            f"The above incident falls under the purview of the Bharatiya Nyaya Sanhita (BNS) 2023. "
            f"Applicable sections have been identified based on the nature of the offense described."
        )

        draft = FIRDraftResponse(
            ai_narrative=mock_narrative,
            recommended_sections=mock_sections,
            extracted_entities={
                "note": "Entity extraction will be available after Phase 7 (AI Integration)",
            },
